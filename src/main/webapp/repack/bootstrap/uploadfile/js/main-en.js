	...
	$('#freeshare-avatarform').fileUploadUI({
        ...
        beforeSend: function (event, files, index, xhr, handler, callBack) {
            ...
            if (!regexp.test(files[index].name)) {
                handler.uploadRow.find('.file_upload_progress').html('The type of picture must be "jpg, gif or bmp"!');
                setTimeout(function () {
	...
	$('#freeshare-avatarfileform').fileUploadUI({
        ...
        beforeSend: function (event, files, index, xhr, handler, callBack) {
            ...
            if (files[index].size > 20*1024*1024) {
                handler.uploadRow.find('.file_upload_progress').html('&nbsp;&nbsp;The size of file must be less than 20MB!');
                setTimeout(function () {
                     handler.removeNode(handler.uploadRow);
                }, 3000);
                return;
        ...
        buildDownloadRow: function (file) {
			...
			if(file.docUuid == null || file.docUuid == ""){//action执行后传回来的uuid
               ...
               return $('<tr><td>Upload failed, please retry!<\/td><\/tr>');
            }else{
            	var $newfilename ;
            	...
            	if($('#freeshare-avatarfilelist .deletefronlist').length === 0)
            	{
                    ...
                    $newfilename = $('<tr><td id="freeshare-firstline" style="padding-left:10px;"><label>Uploaded:</label></td><td>'+file.docFileName+'&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" class="deletefronlist">[delete]</a></td></tr>');
            	}else{
            		...
            		else $newfilename = $('<tr><td></td><td>'+file.docFileName+'&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" class="deletefronlist">[delete]</a></td></tr>');
            	}
	  ...
	  if($(this).parent().parent().index() === 0){
		  ...
		  $.ajax({
			  ...
			  success: function(){
				  ...
				  $('#freeshare-avatarfilelist tr').eq(0).find('td').eq(0).html('<td id="firstline" style="padding-left:10px;"><label>Uploaded:</label></td><td>');
			  },
			});	
		  $(this).parent().parent().remove();
		  $('#freeshare-avatarfilelist tr').eq(0).find('td').eq(0).html('<td id="firstline" style="padding-left:10px;"><label>Uploaded:</label></td><td>');
